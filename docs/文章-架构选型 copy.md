...
### 1.2 当前 AI 开发最主流的几个 npm 包

目前 Node.js 生态里做 AI 应用开发，最常见的几个包大概是这样的：

| 包名 | 定位 | 核心能力 | 适用场景 |
|------|------|---------|---------|
| `@langchain/openai` + `@langchain/community` | LLM 应用框架 | 模型抽象、文档加载、文本分割、链式调用、Agent | 需要文档处理、检索增强、多步推理的完整应用 |
| `ai` (Vercel AI SDK) | 流式传输与 UI 协议 | 统一流式协议、前端组件、多模型 Provider | 需要前后端流式对话、多模型切换的 Web 应用 |
| `@ai-sdk/vue` | AI SDK 的 Vue 适配 | `useChat` composable、消息状态管理 | Vue 前端接入流式对话 |
| `langchain` (聚合包) | LangChain 全家桶入口 | 汇总子包导出 | 快速原型（生产建议用子包按需引入） |

几个关键对比：

**协议支持方面**，LangChain 的 `ChatOpenAI` 实现了 OpenAI 兼容协议。这个协议现在事实上是行业标准——阿里云通义千问、百度文心、DeepSeek、Moonshot、智谱 GLM，几乎所有国产大模型平台都提供了 OpenAI 兼容接口。这意味着你只需要改一个 `baseURL` 和 `apiKey`，就能在不同模型之间切换，不用改业务代码。这个兼容性是选择 LangChain 的一个重要原因。

**架构能力方面**，LangChain 生态提供了 LangGraph（多 Agent 工作流）、LLMGraphTransformer（结构化抽取）、各种 Document Loader 和 Text Splitter。如果你要做 RAG，这些开箱即用的组件能省大量时间。比如 PDF 解析，LangChain 社区提供了 `PDFLoader`、`DocxLoader`、`PPTXLoader`、`CSVLoader`，直接用就行。

**Vercel AI SDK** 的定位不太一样。它不是用来做文档处理或链式推理的，它的核心价值在于**统一了前后端的流式通信协议**。后端用 `createUIMessageStream` 生成标准格式的流，前端用 `useChat` 消费这个流，中间不需要手写 SSE 解析逻辑。这个协议的普及范围后面会详细说。

**所以在这个项目里，两者的关系是互补的**：LangChain 负责后端的模型调用、文档处理和检索逻辑，AI SDK 负责前后端之间的流式通信。项目中后端的 `@ai-sdk/langchain` 桥接包，就是专门用来把 LangChain 的流式输出转换成 AI SDK 的 UI Message Stream 格式的。
...
还有一类方案是高度集成的多 Agent 框架，比如 OpenAI 的 Agents SDK、开源的 DeepAgents 等。它们封装了更完整的 Agent 循环、工具调用和任务编排。但这类框架的集成度高也意味着灵活度低——你想替换其中某个组件（比如换一个向量库或检索策略）会比较困难。对于这个项目来说，我需要从文件上传到 RAG 检索再到流式回答全链路可控，所以选择了更底层的 LangChain + AI SDK 组合，而不是高度集成的 Agent 框架。
