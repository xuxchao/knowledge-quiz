...
const elasticsearchHost = getEnv('ELASTICSEARCH_HOST', 'http://localhost:9200').replace(/\/$/, '');
...
async function resetPostgresDatabase(database, excludedTables = []) {
  ...
  const excludedArray =
    excludedTables.length > 0 ? `ARRAY[${excludedTables.map(quoteSqlString).join(',')}]::text[]` : 'ARRAY[]::text[]';
      ...
      (response) => {
        ...
        response.on('end', () => {
          if (response.statusCode >= 200 && response.statusCode < 300) {
            ...
            reject(new Error(`${method} ${url} 请求失败，状态码: ${response.statusCode}，响应: ${responseBody}`));
          }
        });
      },
    );
...
function buildClickHouseUrl() {
  const url = new URL(`${clickhouseConfig.protocol}://${clickhouseConfig.host}:${clickhouseConfig.port}/`);
...
async function clickHouseQuery(sql) {
  return httpRequest('POST', buildClickHouseUrl(), sql, { 'Content-Type': 'text/plain; charset=utf-8' });
}
...
FORMAT TSVRaw
`;
  const tableNames = (await clickHouseQuery(tableListSql)).split(/\r?\n/).map((name) => name.trim()).filter(Boolean);
...
async function resetElasticsearch() {
  ...
  const body = await httpRequest('GET', `${elasticsearchHost}/_cat/indices?h=index`);
  const indices = body.split(/\s+/).filter(Boolean);

  for (const index of indices) {
    await httpRequest('DELETE', `${elasticsearchHost}/${encodeURIComponent(index)}`);
  }
}
...
function isMissingBucketError(error) {
  return error?.$metadata?.httpStatusCode === 404 || ['NotFound', 'NoSuchBucket', 'NotFoundError'].includes(error?.name);
}
...
async function clearBucket(label, config) {
  ...
  do {
    ...
    const objects = (listResult.Contents || []).map((item) => ({ Key: item.Key })).filter((item) => item.Key);
...
async function checkBucket(label, config) {
  ...
  try {
    ...
    if (isMissingBucketError(error)) {
      console.log(`${label} 存储桶不存在，执行重置时会自动创建: ${config.bucket}`);
      return;
    }
    throw error;
  }
